...
namespace Worse
{
    ...
    void RHIDescriptorSetLayout::nativeCreate()
    {
        ...
        for (Size i = 0; i < m_descriptors.size(); ++i)
        {
        ...
        infoBindingFlags.bindingCount  = static_cast<UInt>(bindingFlags.size());
        ...
        infoLayout.bindingCount = static_cast<UInt>(bindings.size());

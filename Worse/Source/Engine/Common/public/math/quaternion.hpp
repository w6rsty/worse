...
namespace Worse::math
{
    ...
    struct Quaternion
    {
        ...
        Vector3 toEuler() const
        {
            ...
            Float pitch = std::abs(sinp) >= 1.0f ? std::copysign(kPi / 2, sinp) : std::asin(sinp);
        ...
        static Quaternion fromAxisAngle(Vector3 const& axis, Float angle)
        {
            ...
            return Quaternion(Math::Cos(halfAngle), Normalize(axis) * Math::Sin(halfAngle));
        }
    ...
    inline Float magnitudeSquared(Quaternion const& q) { return q.v4.LengthSquared(); }
    inline Float magnitude(Quaternion const& q) { return q.v4.Length(); }

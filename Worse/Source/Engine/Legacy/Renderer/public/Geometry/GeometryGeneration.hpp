...
namespace Worse::geometry
{
    ...
    static void generateQuad3D(std::vector<RHIVertexPosUvNrmTan>& vertices,
                               std::vector<UInt>& indices, Float width = 1.0f,
                               Float height = 1.0f)
    {
        using Worse::Vector2;
        using Worse::Vector3;
    ...
    static void generateSphere(std::vector<RHIVertexPosUvNrmTan>& vertices,
                               std::vector<UInt>& indices, Float radius = 0.5f,
                               UInt segments = 32, UInt rings = 16)
    {
        using Worse::Vector2;
        using Worse::Vector3;
        using Worse::Vector4;
        ...
        const UInt topPoleIndex = 0;

        // Rings (excluding poles)
        for (UInt r = 1; r < rings; ++r)
        {
            Float phi    = kPi * s_cast<Float>(r) / s_cast<Float>(rings);
            Float sinPhi = Math::Sin(phi);
            Float cosPhi = Math::Cos(phi);

            for (UInt s = 0; s <= segments; ++s)
            {
                Float theta    = 2.0f * kPi * s_cast<Float>(s) / s_cast<Float>(segments);
                ...
                Vector2 uv(s_cast<Float>(s) / s_cast<Float>(segments),
                           s_cast<Float>(r) / s_cast<Float>(rings));
                ...
                Vector4 tangent = Vector4(Normalize(Vector3(-sinTheta, 0.0f, cosTheta)), 1.0f);
        ...
        vertices.emplace_back(Vector3(0.0f, -radius, 0.0f),
                              ...
                              Vector3(0.0f, -1.0f, 0.0f),
                              Vector4(1.0f, 0.0f, 0.0f, 1.0f));
        const UInt bottomPoleIndex = s_cast<UInt>(vertices.size() - 1);
        ...
        for (UInt r = 0; r < rings - 2; ++r)
        {
            for (UInt s = 0; s < segments; ++s)
            {
                ...
                UInt first_row_start  = 1 + r * (segments + 1);
                UInt second_row_start = 1 + (r + 1) * (segments + 1);

                UInt i0 = first_row_start + s;
                UInt i1 = first_row_start + s + 1;
                UInt i2 = second_row_start + s;
                UInt i3 = second_row_start + s + 1;
        ...
        UInt lastRingStartIndex = 1 + (rings - 2) * (segments + 1);
        for (UInt s = 0; s < segments; ++s)
        {
            UInt first  = lastRingStartIndex + s;
            UInt second = lastRingStartIndex + s + 1;
    ...
    static void generateCylinder(std::vector<RHIVertexPosUvNrmTan>& vertices,
                                 std::vector<UInt>& indices, Float radius = 0.5f,
                                 Float height = 1.0f, UInt segments = 32,
                                 UInt heightSegments = 1, Bool topCap = true,
                                 Bool bottomCap = true)
    {
        using Worse::Vector2;
        using Worse::Vector3;
        ...
        UInt sideVertices = (segments + 1) * (heightSegments + 1);
        UInt capVertices  = topCap ? (segments + 2) : 0;
        capVertices += bottomCap ? (segments + 2) : 0;

        vertices.reserve(sideVertices + capVertices);

        UInt sideIndices = segments * heightSegments * 6;
        UInt capIndices  = (topCap ? segments * 3 : 0) + (bottomCap ? segments * 3 : 0);
        indices.reserve(sideIndices + capIndices);

        Float halfHeight = height * 0.5f;

        for (UInt h = 0; h <= heightSegments; ++h)
        {
            Float y = -halfHeight + (height * s_cast<Float>(h) / s_cast<Float>(heightSegments));
            Float v = s_cast<Float>(h) / s_cast<Float>(heightSegments);
            ...
            for (UInt s = 0; s <= segments; ++s)
            {
                ...
                Float u     = s_cast<Float>(s) / s_cast<Float>(segments);

                Vector3 position(x, y, z);
                Vector2 uv(u, v);
                Vector3 normal   = Normalize(Vector3(x, 0.0f, z));
                Float handedness = (s % 2 == 0) ? 1.0f : -1.0f;
                Vector4 tangent  = Vector4(Normalize(Vector3(-Math::Sin(theta), 0.0f, Math::Cos(theta))), handedness);
        ...
        UInt currentVertexIndex = sideVertices;
        ...
        if (topCap)
        {
            ...
            UInt topCenterIndex = currentVertexIndex;
            ...
            for (UInt s = 0; s <= segments; ++s)
            {
                ...
                Vector2 uv(0.5f + 0.5f * Math::Cos(theta),
                           0.5f + 0.5f * Math::Sin(theta));
        ...
        if (bottomCap)
        {
            ...
            UInt bottomCenterIndex = currentVertexIndex;
    ...
    static void generateCapsule(std::vector<RHIVertexPosUvNrmTan>& vertices,
                                std::vector<UInt>& indices, Float radius = 0.5f,
                                Float height = 2.0f, UInt segments = 32,
                                UInt totalRings = 16)
    {
        using Worse::Vector2;
        using Worse::Vector3;
        ...
        const Float cylinderHeight     = Max(0.0f, height - 2.0f * radius);
        const Float halfCylinderHeight = cylinderHeight * 0.5f;
        const UInt hemisphereRings     = totalRings / 2;

        // --- Arc-length calculation for proportional V coordinate ---
        const Float hemisphereArcLength = kPi * 0.5f * radius;
        const Float totalArcLength      = 2.0f * hemisphereArcLength + cylinderHeight;
        ...
        UInt rings = hemisphereRings * 2 + 1; // total rings for the whole shape
        if (cylinderHeight > 0)
            rings += 1; // an extra ring for the cylinder middle if it exists

        for (UInt r = 0; r <= rings; ++r)
        {
            Float v_ratio        = s_cast<Float>(r) / s_cast<Float>(rings);
            ...
            if (currentArcDist < hemisphereArcLength) // Top hemisphere
            {
                ...
                effectiveRadius = radius * Math::Sin(phi);
                y               = radius * Math::Cos(phi) + halfCylinderHeight;
                normal_y_dir    = Vector3(0, Math::Cos(phi), 0);
            }

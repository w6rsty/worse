    ...
    void RHIBuffer::nativeCreate(void const* data)
    {
        ...
        // buffer usage and memory properties
        {
            ...
            if ((m_usage & RHIBufferUsageFlagBits::Vertex) ||
                m_usage & RHIBufferUsageFlagBits::Instance) { bufferUsage |= VK_BUFFER_USAGE_VERTEX_BUFFER_BIT; }
            if (m_usage & RHIBufferUsageFlagBits::Index)    { bufferUsage |= VK_BUFFER_USAGE_INDEX_BUFFER_BIT; }
            if (m_usage & RHIBufferUsageFlagBits::Storage)  { bufferUsage |= VK_BUFFER_USAGE_STORAGE_BUFFER_BIT; }
            if (m_usage & RHIBufferUsageFlagBits::Uniform)  { bufferUsage |= VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT; }
    
            // vertex/index/instance only
            if (isVIIOnly)     { bufferUsage |= VK_BUFFER_USAGE_TRANSFER_DST_BIT; }
            // storage only
            if (isStorageOnly) { bufferUsage |= VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT; }
            if (isUniform)     { memoryProperty |= VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT; }
        }
        ...
        m_gpuData = (isUniform && m_mappable)
                        ? RHIDevice::memoryGetMappedBufferData(m_handle)
                        : nullptr;
        // clang-format on
    }
